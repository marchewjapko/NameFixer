...
public class LastNameRepositoryTests
{
    ...
    private static LastNameRepository _repository = null!;

    public LastNameRepositoryTests()
    {
        OptionsMock
    ...
    public void Setup()
    {
        _repository = new LastNameRepository(
            OptionsMock.Object,
    ...
    public async Task ShouldInitialize_ShouldReadFromLocalFile()
    {
        ...
        var femaleNames = new Faker<LastNameDatasetModel>()
            .RuleFor(x => x.LastName, f => f.Name.FirstName(Name.Gender.Female))
            .RuleFor(x => x.NumberOfOccurrences, f => f.IndexFaker)
            .Generate(1000);

        var maleNames = new Faker<LastNameDatasetModel>()
            .RuleFor(x => x.LastName, f => f.Name.FirstName(Name.Gender.Male))
        ...
        FileReaderMock
            .Setup(
                x => x.ReadCsvFile<LastNameDatasetModel>(
        ...
        //Assert
        var numberOfExpected = femaleNames
            .DistinctBy(x => x.LastName)
            .Count(x => x.NumberOfOccurrences >= MinOccurrenceRate) + maleNames
            .DistinctBy(x => x.LastName)
            .Count(x => x.NumberOfOccurrences >= MinOccurrenceRate);
